...
- 補助導線として `S4 Saved Combos` と完了後の `S7 Save Favorite Sheet` を入れる
...
`S3 Curry Detail / Customize` の 2 phase 進行を実装し、`Review Order` から S5 へ渡せる状態にする。
...
`S8 Order Complete`、`S7 Save Favorite Sheet`、再訪導線、アクセシビリティ、動きの整合を仕上げる。
...
- `T15 S7 Save Favorite Sheet` と `T24 Post-Complete Navigation`
- `T25 Motion / Haptics Pass` と `T26 Accessibility / Reduce Motion`
...
- 既存レビューで問題になっているログイン体験は本開発の重要テーマとして扱う
- ただし初版 PoC では、ログインを挟まず注文主導線を評価することを優先する
- `Rewards` や保存同期、注文履歴など、認証が価値になる地点は将来ワークとして別設計する
- 将来ワークでは、`注文開始時にログインを促す` `メール認証を使う` `継続ログインで起動時再認証を避ける` を初期仮説として扱う
