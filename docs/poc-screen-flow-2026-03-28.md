...
- `S1 Store Select` は基本的に閉じるモーダルではなく、戻る履歴に残る stack 画面として扱う
...
- `続けて注文` 後は `S5` を履歴に残さず、`S1 Store Select -> S2 Menu Discovery` の stack を作って S2 を表示する
- `続けて注文` 後の `S2` で戻ると `S1` に戻り、さらに S1 から戻る場合は注文リセット確認を出す
...
- 既存注文がある状態で `S1` から戻る場合も破棄確認を出し、承諾時は店舗と注文状態をリセットして Home に戻す
- 店舗だけ選択済みで注文内容がない状態で `S1` から Home に戻る場合は、確認を挟まず店舗選択をリセットする
...
- 注文確定、受取店舗、受取目安を明示する
- 成功演出は出すが、長すぎない
- 保存提案は各注文内容の金額表示の左側に小さく置き、主CTAを奪わない
