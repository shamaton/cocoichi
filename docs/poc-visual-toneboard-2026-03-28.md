...
- `This Store Only` は限定棚ではなく、短い発見導線として 1 セクションに抑える
- `限定` バッジは強く見せすぎず、暖色アクセントで価値だけを伝える
...
- タグやチップでスパイス色を差す
- 商品価格は `accent.curry`
- `限定` バッジや店舗固有提案は `accent.cheese` または `accent.cream` 寄りで扱い、主CTAの `accent.curry` と競合させない
